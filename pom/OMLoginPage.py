...
class OMLoginPage:
    ...
    def enter_username(self, username):
        ...
        element = WebDriverWait(self.driver, 10).until(EC.presence_of_element_located(self.username))
    ...
    def enter_password(self, password):
        ...
        element = WebDriverWait(self.driver, 10).until(EC.presence_of_element_located(self.password))
        element.send_keys(password)

    def click_login_button(self):
        # element = self.driver.find_element(*self.button_login)
        element = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.button_login))
        element.click()

    def forgot_password(self, forgot_password):
        element = WebDriverWait(self.driver, 10).until(EC.presence_of_element_located(self.forgot_password))
        element.send_keys(forgot_password)
